...
//		throw new IllegalArgumentException("게시글을 찾을 수 없습니다.");
	...
	public void updatePost(Long postId, Post updatePost, MultipartFile file) {
		...
		if (file != null && !file.isEmpty()) {
			...
			if (post.getFileAttachment() != null) {
				FileAttachmentUtil.deleteFile(uploadPath + "/" + post.getFileAttachment().getStoredFilename());
			}
	...
	@Transactional
	public void deleteFile(Long postId, Long fileAttachmentId) {
		Post post = getPostById(postId);
		FileAttachment fileAttachment = post.getFileAttachment();
		
		if (fileAttachment != null && fileAttachment.getId().equals(fileAttachmentId)) {
			FileAttachmentUtil.deleteFile(uploadPath + "/" + fileAttachment.getStoredFilename());
			post.setFileAttachment(null);
		}
	}
